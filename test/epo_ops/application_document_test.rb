  ...
  module Ops
    class BibliographicDocumentTest < Minitest::Test
      def setup
        ...
        application_nr = EpoOps::Util.find_in_data(single_classification_raw, %w(world_patent_data register_search query __content__)).first.partition('=').last
        url = "https://ops.epo.org/3.1/rest-services/register/application/epodoc/#{application_nr}"
        @single_class = EpoOps::BibliographicDocument.new(url: url, raw: single_classification_raw)
        @multi_class = EpoOps::BibliographicDocument.new(url: url, raw: multi_classification_raw)
        @no_agent = EpoOps::BibliographicDocument.new(url: url, raw: no_agent_raw)
        @multi_agent = EpoOps::BibliographicDocument.new(url: url, raw: two_agents_raw)
        @agent_5_address = EpoOps::BibliographicDocument.new(url: url, raw: agent_5_address_raw)
      end
